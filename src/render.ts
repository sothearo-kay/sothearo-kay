...
const BP_MEDIUM = 550;
const BP_LARGE = 700;
const BODY_COPY = `I'm Sothearo, A web developer. I create engaging web applications with modern tech. I love reading manga.`;
...
export const link = (props: Props & { index: number }) => (label: string) => {
  const styles = /*css*/ `
		...
		.link__label {
			animation-delay: ${props.index * 1.5}s;
		}
		.link__arrow {
			font-size: 0.75em;
			position: relative;
